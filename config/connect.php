...
$conn = mysqli_connect('localhost' , 'kartik' , 'kartik@1234','bookstore1');
function db_connect(){
		$conn = mysqli_connect("localhost", "kartik", 'kartik@1234', "bookstore1");
		if(!$conn){
...
if(!$conn){
	...
	function select4LatestBook($conn){
		$row = array();
		$query = "SELECT book_isbn, book_image, book_title, book_author FROM books ORDER BY book_isbn DESC";
		$result = mysqli_query($conn, $query);
		if(!$result){
		    echo "Can't retrieve data " . mysqli_error($conn);
		    exit;
